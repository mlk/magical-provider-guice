...
public abstract class RoboticLegsModule extends AbstractModule {
    ...
    public <T> void bindLeg(Class<T> superClass, Class<? extends Annotation> annotation, Class<? extends T> implementation, Consumer<Binder> requirements) {
        install(new LegModule<>(Key.get(superClass, annotation), implementation, requirements));
    }

    public <T> void bindLeg(Class<T> superClass, Annotation annotation, Class<? extends T> implementation, Consumer<Binder> requirements) {
        install(new LegModule<>(Key.get(superClass, annotation), implementation, requirements));
    }
